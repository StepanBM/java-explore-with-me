...
public class NewHitRequest {
    ...
    @Length(max = 100, message = "Название приложения не может привышать длину в 100 символов", groups = CreateValidation.class)
    private String app;

    @NotBlank(message = "Url не может быть пустым", groups = CreateValidation.class)
    @Length(max = 100, message = "Uri не может привышать длину в 100 символов", groups = CreateValidation.class)
    private String uri;

    @NotBlank(message = "Ip адрес не может быть пустым", groups = CreateValidation.class)
    @Length(max = 100, message = "Ip адрес не может привышать длину в 100 символов", groups = CreateValidation.class)
    private String ip;
